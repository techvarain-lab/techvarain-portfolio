...
— n05 flows carry per-flow `imgs[]` (New Client Onboarding→auto01 + make-onboarding.png; Abandoned Checkout Recovery→auto02/03; Payment Failed Alert→auto04/05) that scope both the preview pane and drawer automation zoom to the focused flow, plus per-flow `problem/solution/outcome` rendered when a flow is focused in the drawer). Items without `img` use `letter` fallback. |
