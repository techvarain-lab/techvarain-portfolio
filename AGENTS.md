...
| 781–803 | Work panel: `.work-toolbar` filters (All 17 / Funnel 6 / Automation 21) + `#workSearch` box, `#rows` bench, `#preview` pane. Under the `automation` filter rows flatten into individual automations (21 flows across 11 builds): `data-id` = `p.n·idx`, parent build as an eyebrow line, flow tags as pills; All/Funnel stay project-level. Search filters within the current filter (flow name/short/tags/parent on automation; title/desc/stack/tags otherwise). Preview column: pinned `.preview-top`, then `.preview-body` is the single page-scroller housing the media + typography. Multi-image projects show the full-page shot at natural size (`.prev-full`, scrolls with the column, click → lightbox); single-image/letter previews keep `.prev-hero` (scale-to-fit `max-height:40vh`). Sticky `▾ more below` cue when overflowing |
